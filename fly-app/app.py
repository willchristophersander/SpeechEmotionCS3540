...
from ser.models import CRNN_6Class, EMOTIONS_6CLASS
...
EMOTIONS = EMOTIONS_6CLASS  # ['Anger', 'Happy', 'Neutral', 'Sad', 'Fear', 'Surprise']
EMOTION_COLORS = {
    'Anger': '#FF4444', 
    'Happy': '#FFD700', 
    'Neutral': '#4A90D9', 
    'Sad': '#6B7B8C',
    'Fear': '#9B59B6',  # Purple for fear
    'Surprise': '#FF6B35'  # Orange for surprise
}
...
def load_model():
    ...
    checkpoint_dir = Path(__file__).parent / 'checkpoints' / '6class'
    ...
    regular_path = checkpoint_dir / 'crnn_multi_dataset_6class.pth'
    quantized_path = checkpoint_dir / 'crnn_multi_dataset_6class_quantized.pth'
    
    if regular_path.exists():
        print("Loading CRNN 6-class model (full precision for accuracy)...")
        model_path = regular_path
        is_quantized = False
    elif quantized_path.exists():
        print("Loading quantized CRNN 6-class model (optimized, may have slight accuracy loss)...")
    ...
    # Create model - use CRNN_6Class which is configured for 6 classes
    model = CRNN_6Class(n_mels=N_MELS)
...
def predict():
    ...
    try:
        ...
        print(f"  Probs: A:{probs[0]*100:.1f}% H:{probs[1]*100:.1f}% N:{probs[2]*100:.1f}% S:{probs[3]*100:.1f}% F:{probs[4]*100:.1f}% Su:{probs[5]*100:.1f}%", flush=True)
...
def health():
    ...
    from pathlib import Path
    model_path = Path(__file__).parent / 'checkpoints' / '6class' / 'crnn_multi_dataset_6class.pth'
    
    return jsonify({
        'status': 'ok',
        'model_loaded': model is not None,
        'model_type': 'CRNN_6Class',
...
if __name__ == '__main__':
    ...
    load_model()
    print("\n" + "="*50)
    print("Speech Emotion Recognition Demo (CRNN 6-Class)")
    print("="*50)
