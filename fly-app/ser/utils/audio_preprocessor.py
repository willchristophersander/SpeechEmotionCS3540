...
class AudioPreprocessor:
    ...
    def process(self, audio, sr=None):
        ...
        # Use fast fallback trimming to avoid numba/librosa timeout issues
        # librosa.effects.trim can hang/crash with numba on some Python versions
        # Simple energy-based trimming is faster and more reliable for production
        rms = np.sqrt(np.mean(audio ** 2))
        threshold = max(rms * 0.1, 0.01)  # 10% of RMS, minimum 0.01
        non_silent = np.where(np.abs(audio) > threshold)[0]
        if len(non_silent) > self.sample_rate * 0.3:  # At least 0.3s
