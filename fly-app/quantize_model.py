#!/usr/bin/env python3
"""
Quantize the trained CRNN 6-class model to reduce size and speed up inference.

This script:
1. Loads the trained 6-class model checkpoint
...
from ser.models import CRNN_6Class
from ser.data.dataset import N_MELS

def quantize_model():
    """Quantize the trained 6-class model to INT8."""
    
    # Paths
    checkpoint_dir = Path(__file__).parent / 'checkpoints' / '6class'
    input_path = checkpoint_dir / 'crnn_multi_dataset_6class.pth'
    output_path = checkpoint_dir / 'crnn_multi_dataset_6class_quantized.pth'
    ...
    print("Quantization complete!")
    print("=" * 60)
    print("\nThe quantized model will be automatically used by app.py if available.")
